...
export default function useCarouselUtils(carouselData){
  ...
  const addRemoveClasslist = (removeList, addList, className_ = "current-slide")=>{
  ...
  const handleDotsClick = (index)=>{
    if (carouselData.length>1){
      const [track, slides, navDots, currentSlide, currentNavDot] = getHtmlElements()
      addRemoveClasslist([currentSlide, currentNavDot],[slides[index], navDots[index]])
  ...
  const changeSlide = (buttonSide="right")=>{
    if (carouselData.length>1){
      const [track, slides, navDots, currentSlide, currentNavDot] = getHtmlElements()
      const nextSlide= buttonSide==="left"?(currentSlide.previousElementSibling?
        currentSlide.previousElementSibling:slides[slides.length-1])
        :(currentSlide.nextElementSibling? currentSlide.nextElementSibling:slides[0])
      // const currentNavDot = nav.querySelector(".current-slide")
      const nextNavDot= buttonSide==="left"?(currentNavDot.previousElementSibling?
        currentNavDot.previousElementSibling:navDots[navDots.length-1])
        :(currentNavDot.nextElementSibling? currentNavDot.nextElementSibling:navDots[0])
      const amountToMove = nextSlide.style.left
      track.style.transform = `translate(-${amountToMove})`
      addRemoveClasslist([currentSlide, currentNavDot], [nextSlide, nextNavDot])
    }
  }
  ...
  const handleClick = (e)=>{
    setCounter(0)
    const buttonSide = e.target.className.includes("left")?"left":"right"
