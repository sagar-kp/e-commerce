...
export default function useFilterResults(searchResults, setDisplayData){
  const [selected, setSelected] = useState({category:"", price:{min:-1, max:-1}, rating:-1})
  ...
  useEffect(()=>{
    ...
    const filterByPrice=(min, max)=>{
      if (typeof min==="number" && typeof max==="number" && max>=0 && min>=0 ){    
        tempDisplayArr = tempDisplayArr.filter(obj=>{
          let price = parseFloat(obj.discounted_price.split('₹')[1].replaceAll(',', ''))
    ...
    if (selected.category.length!==0||selected.rating!==-1||(selected.price.min!==-1 && selected.price.max!==-1)){
